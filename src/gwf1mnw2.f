...
c
      SUBROUTINE GWF1MNW2DF(LCHANI,LCHK,LCHKCC,LCHUFTHK,LCHY,LCSSHMN,
     &                      LCTRPY,LCVKA)
...
c
     ...
     +       in,iout,IWL2CB,MNWPRNT,NODTOT,IFREFM,NLAY,NMNWVL)
C     VERSION 20090405 GZH  20091202 LFK
c
c----- MNW2 
c     ******************************************************************
...
c     ------------------------------------------------------------------
     ...
     &        in,iout,IWL2CB,MNWPRNT,NODTOT,IFREFM,NLAY,
...
c1------identify package and initialize mnwmax
    ...
    1 format(/,1x,'MNW2 -- MULTI-NODE WELL PACKAGE, VERSION 2,',
     +' 12/18/2009.',/,4X,'INPUT READ FROM UNIT ',i3)
      ...
      ntotnod=0
c
c2------read max number of mnw2 wells,
...
c  31 is the number of records in MNWNOD
      ispmnwn  = 31 * NODTOT   
      isumz = isumz+ispmnwn
...
c
      ...
      dimension mnw2(NMNWVL,mnwmax),MNWNOD(31,NODTOT),MNWINT(11,NODTOT)
...
c     write output with Format depending on LOSSTYPE
     ...
     & PUMPCAP
         ...
         CALL USTOP('MNW2 ERROR - LOSSTYPE')
        end if
...
c     for none, NNODES must be 1
          ...
          write(iout,*) '***ERROR***  OPTION: NONE   REQUIRES NNODES=1'
            CALL USTOP('MNW2 ERROR - OPTION: NONE  REQUIRES NNODES=1')
          end if
          MNW2(3,MNWID)=0
        elseif(LOSSTYPE.EQ.'THIEM') then
          MNW2(3,MNWID)=1 
...
c     count nodes (will uses this to check vs. allocation)
          ...
          if(ntotnod.gt.nodtot) then
            ...
            CALL USTOP('MNW2 ALLOCATION ERROR')
          end if
c     loop over nodes 
          DO 3 INODE=1,NNODES
...
c     If beyond the first interval, do check on elevation
     ...
     &           'above bottom of last interval'                
                ...
                CALL USTOP('MNW2 ERROR - Intervals')
              END IF
            END IF
...
c     save IR and IC to check vs the subsequent nodes
     ...
     &            'a vertical well (NNODES < 0)'
                CALL USTOP('MNW2 ERROR - Vertical')
              END IF
            END IF
c
...
c     until ZDP (and ZPL) are set for the well)
              ...
              ELSE
                ...
                CALL USTOP('MNW2 - screen in no-flow node')
              END IF
...
c     check consistency of CapTable
     ...
     &must be in descending order'
            CALL USTOP('MNW2 ERROR - CapTable')
          end if
...
c     error check Qfrcmn
          if(QCUT.LT.0) then
           if(Qfrcmn.gt.1.d0) then
            write(iout,*) '***ERROR*** Qfrcmn > 1 is out of range'
            CALL USTOP('MNW2 ERROR - Qfrcmn > 1')
           end if
           if(Qfrcmn.gt.Qfrcmx) then
            write(iout,*) '***ERROR*** Qfrcmn > Qfrcmx '
            CALL USTOP('MNW2 ERROR - Qfrcmn > Qfrcmx')
           end if
c     error check Qfrcmx
           if(Qfrcmx.gt.1.d0) then
            write(iout,*) '***ERROR*** Qfrcmx > 1 is out of range'
            CALL USTOP('MNW2 ERROR - Qfrcmx > 1')
           end if
          end if
        END IF
c
...
C     ------------------------------------------------------------------
     ...
     & itflag,INBCF,ssflag,LAYHDT,NNODES,kkper,MNWPRNT,NMNWVL,nd
     ...
     & MNWINT,QCUT,qnet
      dimension mnw2(NMNWVL,MNWMAX),MNWNOD(31,NODTOT),MNWINT(11,INTTOT)
...
C       print warning if found
     ...
     &hould not exist in same cell as a multi-node well'
                 write(iout,*) 'Condition found for node ',nd,' in well 
     *',wellid(iw),' at Lay-Row-Col Index = ',il,ir,ic
              end if
C-LFK   Check for MNW & specified-head boundary condition in same cell; 
C       print warning if found
              if(ibound(ic,ir,il).lt.0) then
...
C-LFK                write(iout,*) 'In Well ',WELLID(iw),' Node ',INODE
                cond=0.d0
              end if
...
c-lfk
            call MNW2CAPACITY(mnw2,qactCap,WELLID,mnwmax,Hlift,
     & hwell,iw,PUMPCAP,CapTable,iout,NMNWVL)
...
c     ------------------------------------------------------------------
      IMPLICIT NONE
      INTEGER ibd,IWL2CB,ICBCFL,NAUX,NCOL,NROW,NLAY,KSTP,KPER,nmnw2,
     & iout,ibound,MNWMAX,imult,iw,firstnode,lastnode,inode,il,ir,ic,
     & NODTOT,ioch,ipole,ioc,NMNWVL,msum,nd,iweldry,MNWPRNT,ntotnod,
     & NBOTM,LBOTM,LAYCBD
      REAL HDRY, VBVL, BUFF, PERTIM, TOTIM, DELT, BOTM
     ...
     & buff(NCOL,NROW,NLAY),MNWNOD(31,NODTOT),hnew(NCOL,NROW,NLAY),
     & vbvl(4,msum),BOTM(NCOL,NROW,0:NBOTM)
      dimension WELLID(mnwmax)
      COMMON /MNW2COM/MNWAUX(5)
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
      CHARACTER*16 MNWAUX
...
C     ------------------------------------------------------------------
     ...
     & hnew(NCOL,NROW,NLAY),MNWNOD(31,NODTOT),mnw2(NMNWVL,MNWMAX)
...
C     ------------------------------------------------------------------
     ...
     & MNWNOD(31,NODTOT),mnw2(NMNWVL,MNWMAX)
      DIMENSION BOTM(NCOL,NROW,0:NBOTM),
...
c   determine well characteristics for nonvertical wells
     ...
     & MNWMAX,IW,kkstp,kkper,ipr,alpha,INBCF,VKA,HK,iout,nmnw2)
        else
...
c     
     ...
     & LAYCBD,NMNWVL,kkstp,iout,mnwprnt,nd
      REAL BOTM,hdry
     ...
     & MNWNOD(31,NODTOT),IBOUND(NCOL,NROW,NLAY),hnew(NCOL,NROW,NLAY)
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
C
      verysmall = 1.D-25
      Qsmall = small
...
c-lfk
c      if (mnw2(20,iw).ge.1.and.abs(mnw2(30,iw)).gt.verysmall) 
c     * mnw2(30,iw)=Qpot              
              mnw2(20,iw) = 2
             else
c  Otherwise leave the flow rate alone
...
c
     ...
     & nodepump,ifound,ir,irp,il,ilp,ic,icp,iout,NMNWVL
      double precision mnw2,Qnet,MNWNOD,diff,q
      dimension MNWNOD(31,NODTOT),mnw2(NMNWVL,MNWMAX)
...
c get pump coordinates
     ...
     & not found, MNW2'
            CALL USTOP('MNW2 ERROR - PUMPLOC2')
          end if
        end if
c get qnet
        Qnet=0.D0
...
C     ******************************************************************
     ...
     & m,lxf,lyf,lzf,lbf,
     & zwt,ywt,xwt,
     & zi,yi,xi,zi2,yi2,xi2,HK,VKA,t1,b1,zseg1,zseg2
...
C     ******************************************************************
C
      SUBROUTINE MNW2CAPACITY(mnw2,qactCap,WELLID,mnwmax,Hlift,hwell,iw,
     & PUMPCAP,CapTable,iout,NMNWVL)
C
C     ******************************************************************
      IMPLICIT NONE
      CHARACTER*20 WELLID
      INTEGER mnwmax,iw,PUMPCAP,idone,index,iout,ifirstL,isecondL,NMNWVL
      DOUBLE PRECISION qactCap,LIFTact,Hlift,hwell,CapTable,m,b,
     & L1,L2,Q1,Q2
      DOUBLE PRECISION mnw2,CapMult
...
C---WRITE RESULTS
C
         IF(DDPP.LT.0.00001D0)THEN
          DDPPOLD=0.0D0
          GO TO 30
         ENDIF
C
         DDTEST=(DABS(DDPP-DDPPOLD))/DDPP
